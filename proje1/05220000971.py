...
while diger_kutu=="e" or diger_kutu=="E":
    ...
    for bilye in range(kutudaki_bilye_sayisi):
        ...
        if bilye_mg!=onceki_bilye_mg:
            farkli_bilye_sayisi+=1
            if onceki_bilye_mg!=0:
                bilye_mg_fark=bilye_mg-onceki_bilye_mg
    ...
    if farkli_bilye_sayisi==0:
        ...
        if bilye_mg_fark>0:
            print(f"Farklı bilye diğerlerinden daha ağırdır. Aradaki fark {abs(bilye_mg_fark)} mg, fark yüzdesi %{(bilye_mg_fark/onceki_bilye_mg)*100:.2f}'dir.")
            digerlerinden_agir_kutu+=1
            daha_agir_mg_farki_top+=bilye_mg_fark
            daha_agir_yuzde_top+=(bilye_mg_fark/onceki_bilye_mg)*100
            agirlik="ağır"
        else:
            print(f"Farklı bilye diğerlerinden daha hafiftir. Aradaki fark {abs(bilye_mg_fark)} mg, fark yüzdesi %{(abs(bilye_mg_fark)/onceki_bilye_mg)*100:.2f}'dir.")
            digerlerinden_hafif_kutu+=1
            daha_hafif_mg_farki_top+=bilye_mg_fark
            daha_hafif_yuzde_top=(bilye_mg_fark/onceki_bilye_mg)*100
            agirlik="hafif"
        if abs(bilye_mg_fark)>en_agirlik_fark:
            en_agirlik_fark=abs(bilye_mg_fark)
            en_agirlik_hafiflik=agirlik
            en_agirlik_yuzde=(abs(bilye_mg_fark)/onceki_bilye_mg)*100
        if (abs(bilye_mg_fark)/onceki_bilye_mg)*100<min_agirlik_yuzde:
            min_agirlik_yuzde=(abs(bilye_mg_fark)/onceki_bilye_mg)*100
...
print(f"{hepsi_esit_kutu} tane kutuda bütün bilyeler eşit ağırlıkta ve bu kutuların iade edilmeyen kutular içindeki yüzdesi %{100*(hepsi_esit_kutu/(kutu_no-1)):.2f}'dir")
print(f"{digerlerinden_agir_kutu} tane kutuda bir bilye diğerlerinden daha ağır ve bu kutuların iade edilmeyen kutular içindeki yüzdesi %{100*(digerlerinden_agir_kutu/(kutu_no-1)):.2f}'dir")
print(f"{digerlerinden_hafif_kutu} tane kutuda bir bilye diğerlerinden daha hafif ve bu kutuların iade edilmeyen kutular içindeki yüzdesi %{100*(digerlerinden_hafif_kutu/(kutu_no-1)):.2f}'dir")
